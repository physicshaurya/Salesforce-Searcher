// JSON to store key prefix and object name pair
const KEY_PREFIX_OBJECT_ARRAY = [
  {
    "Key Prefix": "000",
...
// Array to store name of standard objects
const OBJECT_NAME_ARRAY = [
...
// Function to fetch Id or Object name from tab url
function extractIdOrObjectFromUrl(url) {
  // If the url is neigther or classic or lightning implies not a valid url
  var slicedUrl = null;
  if (!url.includes("salesforce.com") && !url.includes("lightning.force.com")) {
    return null;
  } else {
    slicedUrl = url.substring(url.indexOf(".com"));
  }

  var wordList = slicedUrl.split("/");

  for (var i = 0; i < wordList.length; i++) {
    var item = wordList[i];
    if (RECORD_ID_SOBJECT_TYPE_MAP.has(item.slice(0, 3))) {
      if ((item.length === 15 || item.length === 18) && !item.includes("?")) {
        return item; //record Id would be returned
      } else {
  ...
  for (var j = 0; j < wordList.length; j++) {
    var item = wordList[j];
    if (OBJECT_RECORD_ID_START_CHAR_MAP.has(item.toLowerCase())) {
      return item; //Object Name would be returned
    }
  }
...
// Fetch URL of current tab
chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
  var currentTab = tabs[0];
  var currentTabUrl = currentTab.url;

  if (currentTabUrl) {
    const EXTRACTED_ID_OR_OBJECT = extractIdOrObjectFromUrl(currentTabUrl);
    if (EXTRACTED_ID_OR_OBJECT && SEARCH_INPUT) {
      SEARCH_INPUT.value = EXTRACTED_ID_OR_OBJECT;

      // Manually trigger the input event so that the result is displayed and documentation is shown
...
// Create a DOC_BUTTON element
const DOC_BUTTON = document.createElement("button");
DOC_BUTTON.textContent = "Documentation"; 
DOC_BUTTON.classList.add("styled-button"); 

// Add an event listener to the DOC_BUTTON
DOC_BUTTON.addEventListener("click", () => {
  // Open the URL in a new tab
  window.open(linkToRedirectOnButtonClick, "_blank");
});

const OBJECTNAMESET = new Set(OBJECT_NAME_ARRAY); // Creating a set to store the name of standard objects

// Create Map directly from obj array using map function
const RECORD_ID_SOBJECT_TYPE_MAP = new Map(
  KEY_PREFIX_OBJECT_ARRAY.map((item) => [item["Key Prefix"], item["Object Type"]])
);

// Create another Map with swapped key-value pairs
const OBJECT_RECORD_ID_START_CHAR_MAP = new Map(
  KEY_PREFIX_OBJECT_ARRAY.map((item) => [
    item["Object Type"].toLowerCase(),
    item["Key Prefix"],
  ])
);
...
let buttonFlag = false; // Flag to keep track if button is already in dom or not
let linkToRedirectOnButtonClick = null;

let timeoutId = null; // Variable to store the timeout
const TIME_OUT_TIME = 0.2; // Timeout time in seconds

SEARCH_INPUT.addEventListener("input", () => {
  // Clear any previous timeout
  clearTimeout(timeoutId);

  // Set a new timeout to trigger search after 200ms of user stopping typing
  timeoutId = setTimeout(() => {
    const TRIMMED_INPUT = SEARCH_INPUT.value.trim();
    const SEARCH_VALUE = TRIMMED_INPUT.slice(0, 3);
    let foundMap = null;
    let foundValue = null;
    let objectName = null;

    if (RECORD_ID_SOBJECT_TYPE_MAP.has(SEARCH_VALUE)) {
      foundMap = "RECORD_ID_SOBJECT_TYPE_MAP";
      foundValue = RECORD_ID_SOBJECT_TYPE_MAP.get(SEARCH_VALUE);
      objectName = foundValue;
    } else if (OBJECT_RECORD_ID_START_CHAR_MAP.has(TRIMMED_INPUT.toLowerCase())) {
      foundMap = "OBJECT_RECORD_ID_START_CHAR_MAP";
      foundValue = OBJECT_RECORD_ID_START_CHAR_MAP.get(TRIMMED_INPUT.toLowerCase());
      objectName = RECORD_ID_SOBJECT_TYPE_MAP.get(foundValue);
    }

    if (foundMap == "RECORD_ID_SOBJECT_TYPE_MAP" && TRIMMED_INPUT) {
      OUTPUT_DIV.textContent = `Object is : ${foundValue}`;
      if (OBJECTNAMESET.has(objectName)) {
        if (!buttonFlag) {
          OUTPUT_DIV.insertAdjacentElement("afterend", DOC_BUTTON); // Insert the DOC_BUTTON next to the div
          buttonFlag = true;
        }
        linkToRedirectOnButtonClick = `https://developer.salesforce.com/docs/atlas.en-us.object_reference.meta/object_reference/sforce_api_objects_${objectName.toLowerCase()}.htm`;
      }
    } else if (foundMap == "OBJECT_RECORD_ID_START_CHAR_MAP" && TRIMMED_INPUT) {
      OUTPUT_DIV.textContent = `Record Id starts with : ${foundValue}`;
      if (OBJECTNAMESET.has(objectName)) {
        if (!buttonFlag) {
          OUTPUT_DIV.insertAdjacentElement("afterend", DOC_BUTTON); // Insert the DOC_BUTTON next to the div
          buttonFlag = true;
      ...
      OUTPUT_DIV.textContent = "Provide a valid input";
      if (buttonFlag) {
        DOC_BUTTON.parentNode.removeChild(DOC_BUTTON); // Remove the DOC_BUTTON from flow
        buttonFlag = false;
      }
    }
  }, TIME_OUT_TIME * 1000);
